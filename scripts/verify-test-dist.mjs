...
for (const name of refNames) {
  if (resolveWrittenName(name, outNames) !== name)
    aliasedStems.add(refStem(name));
}
...
for (const name of refNames) {
  ...
  if (outMeta.width !== refMeta.width || outMeta.height !== refMeta.height) {
    console.error(
      `${match}: ${outMeta.width}x${outMeta.height} != ${refMeta.width}x${refMeta.height}`,
    );
    process.exit(1);
  }
  if (!outMeta.exif) {
...
console.log(
  `ok: ${refNames.length} reference file(s) matched in dist (names + dimensions + EXIF)`,
);
