...
export async function assertFfmpegOnPath(): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    proc.on('error', () => reject(new Error('ffmpeg not found on PATH (required for clips commands)')));
    proc.on('close', (code) => {
      if (code === 0) resolve();
      else reject(new Error(`ffmpeg -version exited with code ${code ?? 'unknown'}`));
    });
  });
}
...
export async function rasterizeWatermarkForVideo(
  ...
  const margin = Math.max(12, Math.round(Math.min(frameWidth, frameHeight) * options.paddingRatio));
  ...
  const outPath = path.join(path.dirname(watermarkPath), `.graphy-wm-${process.pid}.png`);
...
export async function overlayWatermarkOnClip(input: ClipWatermarkInput): Promise<void> {
