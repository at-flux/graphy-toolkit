...
function defaultBinDir(): string {
  if (process.platform === 'win32') {
    return path.join(process.env.LOCALAPPDATA ?? path.join(os.homedir(), 'AppData', 'Local'), 'graphy', 'bin');
  }
...
async function runNpxSkills(args: string[]): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      if (code === 0) resolve();
      else reject(new Error(`npx skills add exited with code ${code ?? 'unknown'}`));
    });
  });
}
...
async function installBinary(binDir: string): Promise<string | null> {
  ...
  const dest = path.join(binDir, process.platform === 'win32' ? 'graphy.exe' : 'graphy');
...
const installAllCommand = buildCommand({
  func: async (flags: InstallFlags) => {
    ...
    if (installed) {
      ...
      console.log('no compiled binary at dist/bin/graphy — run pnpm compile:graphy or npm i -g @at-flux/graphy-toolkit');
      pathHint(binDir);
    }
  },
  parameters: {
  ...
  docs: { brief: 'Install skill and copy graphy binary to PATH (default for graphy install)' },
});
