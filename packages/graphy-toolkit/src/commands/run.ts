...
import type { RunMediaResult } from '@at-flux/graphy-toolkit-core';
...
export function startRun(command: string, resolved: ResolvedMedia): void {
  ...
  process.stderr.write(`  ${dim('source')}     ${bold(String(n))} ${n === 1 ? 'file' : 'files'} — ${rel(cwd, source.sourceRoot)}\n`);
  process.stderr.write(`  ${dim('output')}     ${rel(cwd, distRoot)}\n`);
  if (presetsPath) {
    process.stderr.write(`  ${dim('presets')}    ${rel(cwd, presetsPath)} ${green('loaded')}\n`);
  }
  process.stderr.write(`  ${dim('pipelines')} ${settings.pipelines.join(', ')}\n`);

  const applied: string[] = [];
  if (settings.watermark) {
    applied.push(`watermark ${rel(cwd, path.resolve(cwd, settings.watermark.path))}`);
    applied.push(`opacity ${Math.round(settings.watermark.opacity * 100)}%`);
    applied.push(`mark size ${Math.round(settings.watermark.sizeRatio * 100)}%`);
  }
  if (settings.jpegQuality != null) applied.push(`jpeg quality ${settings.jpegQuality}`);
...
export function finishRun(
  ...
  if (!quiet && result.errors.length > 0) {
    ...
    for (const e of result.errors) {
      const where = e.pipeline ? `${path.basename(e.file)} [${e.pipeline}]` : path.basename(e.file);
  ...
  console.log(`done: ${result.processed} input(s), ${uniqueWritten} file(s) written`);
